...
The shared modeling primitives this CLI is built on (build_panel, market_panel,
feature_names, weekly_predict, feature_scan, settle/score/roi_interval,
cutoff_grid, apply_representation, symmetrize_training) live in backtester.py
now -- imported back here, and still reachable as e.g. optimize_picks.settle
for anything (tests, shared_scoring.py, joint_scoring.py, weekly_packet.py,
joint_feature_selection.py, main.py) already written against this module.
research()/confirm_neural()/production_rule_scan()/rescore_saved() are this
file's own thing: a specific CLI-driven research workflow, not primitives
...
from backtester import (
    KEY, MODEL, PRODUCTION_FEATURES, dc,
    build_panel, symmetric_features, market_panel, feature_names,
    weekly_predict, policy_sd_cutoff, neural_spec, settle, score,
    roi_interval, feature_scan, cutoff_grid,
)
...
def production_rule_scan(args):
    ...
    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=True)
    panel = build_panel(args.season, args.week, args.history_weeks, 20, 'mean')
...
def main(argv=None):
    ...
    parser.add_argument('--calculations', nargs='+', choices=['weighted', 'steep', 'mean', 'median'],
                        default=['weighted', 'steep', 'mean', 'median'])
