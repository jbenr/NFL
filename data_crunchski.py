...
def prep_test_train(szn, week, lookback):
    ...
    data = [] # where we gather our final rows of data per game

    tings = df.groupby(['season', 'week']).agg('count').index.tolist()
    print(tings)
    for s, w in tqdm(tings, desc="Crunching the numbers", unit="item"):
    # for s, w in df.groupby(['season','week']).agg('count').index:
